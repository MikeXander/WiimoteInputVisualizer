# Source: https://github.com/yarolig/OBJFileLoader (http://www.pygame.org/wiki/OBJFileLoader)
# Addition: this version allows you to scale the obj size
...
class OBJ:
    ...
    def __init__(self, filename, swapyz=False, scale = 1):
        ...
        for line in open(filename, "r"):
            ...
            if not values: continue
            if values[0] == 'v':
                v = list(map(lambda x: scale * float(x), values[1:4]))
                if swapyz:
                    v = v[0], v[2], v[1]
